...
Ball::Ball(){
    ...
    ballImage.load(":Resource/images/Ball.png");
...
void Ball::autoMove(){
    ...
    // Allows the Ball to move up until it reaches the top edge of the game space
    if (ball.top() == 0){
        moveY = 1;
    }

    // Allows the Ball to move right until it reaches the left edge of the game space
    if (ball.left() == 0){
        moveX = 1;
    }

    // Allows the Ball to move right until it reaches the right edge of the game space
    if (ball.right() == 330){
        moveX = -1;
    }
}
