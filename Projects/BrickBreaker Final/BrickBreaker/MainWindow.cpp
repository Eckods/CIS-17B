...
void MainWindow::createGameWidget(){
    ...
    // Creates the Brick brick game
    GameManager *brickBreaker = new GameManager();

    // Creates a label above the user score display
    ...
    gameVLayout->addWidget(brickBreaker->scoreDisplay, 0, Qt::AlignHCenter);
    gameVLayout->addWidget(instructions, 0, Qt::AlignHCenter);
    gameVLayout->addWidget(brickBreaker->resetButton, 0, Qt::AlignHCenter);
    gameVLayout->addSpacing(15);
    ...
    gameHLayout->addWidget(brickBreaker, 0, Qt::AlignLeft);
    gameHLayout->addWidget(scoreGroupBox, 0 , Qt::AlignHCenter);

    // Sets the focus to the Brick break game
    setFocusProxy(brickBreaker);
...
void MainWindow::createMenus(){
    ...
    miscMenu->addAction(globalScoresAct);
}
...
void MainWindow::viewGlobalScores(){
    GlobalScoresWindow globalScoresWindow;
    globalScoresWindow.setModal(true);
    globalScoresWindow.setStyleSheet("QDialog{background-image:url(:/Resource/images/Background.png)}");
    globalScoresWindow.exec();
}
